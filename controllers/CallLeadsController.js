...
// //Get call leads...
// //IF callMadeTo Principal fetch data by blockId, if ABRC fetch data by blolckId, if BEO...
// //... fetch data by districtId
...
//... fetch data by districtId
  ...
  try {
    ...
    const { blockId, districtId, callMadeTo } = req.body;
    console.log(req.body)
    ...
    // Get today's date (start of day) and previous 6 days
    const now = new Date();
    const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const sixDaysAgo = new Date(todayStart.getTime() - 6 * 24 * 60 * 60 * 1000);

    // For Principal calls, we need special logic to include previous day's unattempted calls
    if (callMadeTo === "Principal") {
      // Get all schools based on blockId
      if (!blockId) {
        return res.status(400).json({ status: "error", message: "blockId is required for Principal and ABRC calls" });
      }
      
      const blockIds = Array.isArray(blockId) ? blockId : [blockId];
      const schoolsInBlocks = await District_Block_School.find({ 
        blockId: { $in: blockIds } 
      }).select('_id').lean();
      
      const schoolIds = schoolsInBlocks.map(school => school._id);
      match.objectIdOfCalledPerson = { $in: schoolIds };

      // Special date logic for Principal calls
      const pipeline = [
        { $match: match },

        // Add a field to check if callingStatus is null (call not attempted)
        {
          $addFields: {
            isCallNotAttempted: { $eq: ["$callingStatus", null] }
          }
        },

        // Filter: include today's calls OR yesterday's unattempted calls
        {
          $match: {
            $or: [
              // Today's calls (regardless of status)
              { 
                callingDate: { 
                  $gte: todayStart,
              ...
              // Yesterday's unattempted calls
              { 
                callingDate: { 
                  $gte: new Date(todayStart.getTime() - 24 * 60 * 60 * 1000),
                  $lt: todayStart
                },
                isCallNotAttempted: true
              }
            ]
          }
        },

        // Rest of the pipeline remains same...
        // lookup caller user details
        {
          $lookup: {
            from: "users",
      ...
      // For ABRC, BEO, DEO - only show today's calls
      if (callMadeTo === "ABRC") {
        if (!blockId) {
          return res.status(400).json({ status: "error", message: "blockId is required for Principal and ABRC calls" });
        }
      ...
      } else if (callMadeTo === "BEO" || callMadeTo === "DEO") {
        if (!districtId) {
          return res.status(400).json({ status: "error", message: "districtId is required for BEO and DEO calls" });
        }
        
        const districtIds = Array.isArray(districtId) ? districtId : [districtId];
        const schoolsInDistricts = await District_Block_School.find({ 
          districtId: { $in: districtIds } 
        }).select('_id').lean();
        
        const schoolIds = schoolsInDistricts.map(school => school._id);
        match.objectIdOfCalledPerson = { $in: schoolIds };
      }

      // For non-Principal calls, only show today's date
      match.callingDate = { 
        $gte: todayStart,
        $lt: new Date(todayStart.getTime() + 24 * 60 * 60 * 1000)
      };
...
export const UpdateCallLeads = async (req, res) => {
  try {
    ...
    if (mannualRemark !== undefined) update.mannualRemark = mannualRemark;
       if (objectIdOfCaller !== undefined) update.objectIdOfCaller = objectIdOfCaller;
